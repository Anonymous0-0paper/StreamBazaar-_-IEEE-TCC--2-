# StreamBazaar — 10-Node Deployment

Ten machines (or simulated containers on one machine).
Kafka partitions: **10**. 16 tenants distributed across 10 nodes (6 nodes get 2 tenants, 4 nodes get 1 tenant).
...
```
┌─────────────────────────┐
│  node-0 (infra + shard) │   Kafka(10p) · Redis · Postgres
│  tenant-fraud           │   Prometheus · Grafana
│  tenant-iot-4           │   auction-orchestrator
└────────────┬────────────┘
             │ shared Redis + Kafka
  node-0 … node-9  (10 total worker shards)
```
...
| Node | Tenants |
|------|---------|
| node-0 | tenant-fraud, tenant-iot-4 |
| node-1 | tenant-clickstream, tenant-fraud-3 |
| node-2 | tenant-ml, tenant-clickstream-4 |
| node-3 | tenant-iot, tenant-ml-4 |
| node-4 | tenant-fraud-2 |
| node-5 | tenant-web |
| node-6 | tenant-intrusion |
| node-7 | tenant-iot-2 |
| node-8 | tenant-fraud-4 |
| node-9 | tenant-iot-3 |

> The run-distributed.sh script uses `TOTAL_TENANTS = NODE_COUNT * 2` but caps at 16. With `NODE_COUNT=10`, 16 tenants are spread round-robin: nodes 0–5 get 2 tenants, nodes 6–9 get 1 tenant. The exact assignment depends on the round-robin order in the script — verify with `NODE_COUNT=10 bash scripts/run-distributed.sh start` output.

---

## Prerequisites (all 10 machines)
...
NODE_COUNT=10 bash scripts/run-distributed.sh start

# Check all 10 shards
NODE_COUNT=10 bash scripts/run-distributed.sh status
```
...
### Step 3 — Create Kafka topics with 10 partitions
...
|---------|---------|---------|--------------|
| 0 | 18085 | 18081 | tenant-fraud,tenant-iot-4 |
| 1 | 18095 | 18091 | tenant-clickstream,tenant-fraud-3 |
| 2 | 18105 | 18101 | tenant-ml,tenant-clickstream-4 |
| 3 | 18115 | 18111 | tenant-iot,tenant-ml-4 |
| 4 | 18125 | 18121 | tenant-fraud-2 |
| 5 | 18135 | 18131 | tenant-web |
| 6 | 18145 | 18141 | tenant-intrusion |
| 7 | 18155 | 18151 | tenant-iot-2 |
| 8 | 18165 | 18161 | tenant-fraud-4 |
| 9 | 18175 | 18171 | tenant-iot-3 |

> **node-0**: Use `kafka:9092` / `redis://redis:6379/0` (internal Docker hostnames) instead of external IPs.
...
### Step 6 — Verify all 10 nodes

```bash
for port in 18085 18095 18105 18115 18125 18135 18145 18155 18165 18175; do
  echo -n "port ${port}: "
...
|------|---------|-----------------|--------------|
| node-0 | tenant-fraud, tenant-iot-4 | 18085 | 18081 |
| node-1 | tenant-clickstream, tenant-fraud-3 | 18095 | 18091 |
| node-2 | tenant-ml, tenant-clickstream-4 | 18105 | 18101 |
| node-3 | tenant-iot, tenant-ml-4 | 18115 | 18111 |
...
| node-8 | tenant-fraud-4 | 18165 | 18161 |
| node-9 | tenant-iot-3 | 18175 | 18171 |
...
```bash
NODE_COUNT=10 bash scripts/run-distributed.sh stop
```
...
| Symptom | Fix |
|---------|-----|
| A node shows 0 consumed events | Check `TENANT_IDS` env var; confirm Kafka partition count is 10 |
...
| Port conflict on single machine | All ports offset by 10 per node (18085 … 18175) |
